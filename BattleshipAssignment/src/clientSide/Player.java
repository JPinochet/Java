...
public class Player
{
	...
	private int shotsFired; // shots taken
	...
	private int[][] computerMiss = new int[10][10];//used by computer to track miss(0)
	...
	/**
	 * Method returns the users name.
	 * 
	 * @return user. The users NAME
	 */
	...
	/**
	 * Checks if the Statistics frame is open.
	 */
	...
	{
		this.computerMiss[x][y] = z;			
	}
		
	public int getMHS(int x, int y)
	{
		return this.computerMiss[x][y];				
	}	
	
	/**
	 * Method that determines if hit ship is sunk(returns true) or not(false).
	 * 
	 * @param x coordinate.
	 * @param y coordinate.
	 * @return true/fase
	...
	public boolean isSunk(int x, int y)
	{
		...
		if (this.boats[f].getHitsLeft() == 0)
		{
			...
			if ((Battleship.getPlayers(Battleship.getYou()).getUser().equals("Computer"))||(Battleship.getPlayers(Battleship.getYou()).getUser().equals("CPU1"))||(Battleship.getPlayers(Battleship.getYou()).getUser().equals("CPU2")))
			{	
				for (int k = 0; k < 10; k++)
				{
					for (int m = 0; m < 10; m++)
				...
				if ((!this.getUser().equals("CPU1"))&&(!this.getUser().equals("CPU2")))
				{
					JOptionPane.showMessageDialog(null,"You just lost your " + this.boats[f].getName() + "!", "Ship Destroyed", JOptionPane.WARNING_MESSAGE);
				}
			}
			else
			{
				JOptionPane.showMessageDialog(null,"You sank the " + this.boats[f].getName()+"!","Good Job!", JOptionPane.INFORMATION_MESSAGE);
	...
	/**
	 * Method that determines if hit ship is sunk(true) or not(false) passing in the ship in question.
	 * 
	 * @param x passes the x coordinates
	 * @param y passes the y coordinates
	 * @param z passes in the boats name
	 * @return true/false
	 */
	...
	/**
	 * Method checks if any of the surrounding points are plausible
	 * @param x coordinate.
	 * @param y coordinate.
	 * @return
	 */
	...
	/**
	 * Method checks if shot hasn't already been made or if the shot is even possible.
	 * 
	 * @param x coordinates on the grid
	 * @param y coordinates on the grid
	 * @return true/false
	 */
	...
	/**
	 * Method checks if selected position is plausible location.
	 * 
	 * @param x coordinates on the grid
	 * @param y coordinates on the grid
	 * @return true/false
	...
	public boolean rshipsv(int x,int y)
	{
		...
		while (u < 5)				
		{
			...
			{						
				...
				{							
					if ((isValid(i,y))&&((Battleship.getPlayers(Battleship.getYou()).getMHS(i,y)==3) || (Battleship.getPlayers(Battleship.getYou()).getMHS(i,y)==1)))
					{
						g += 1;
	...
	/**
	 * Method check if x, y position where shot was made is correct.
	 * 
	 * @param x coordinate from grid
	 * @param y coordinate from grid
	 * @return true/false
	 */
	...
	/**
	 * Method for the computer to scan the area.
	 * 	
	 * @param x coordinate
	 * @param y coordinate
	 */
	...
	/**
	 * Method passed in an action event class, ae, and checks to see if the players shot if made, where, if it's valid
	 * and than is it passes those checks makes the shot using another method.
	 * 
	 * @param ae actionevent.
	 */
	public void humanAttack(ActionEvent ae)
	{
		if (this.getMove())
		{				
			...
			{				
				...
				{					
					...
					{								
						...
						{
							JOptionPane.showMessageDialog(null,"You tri"+"ed that spot already.","Wasted Shot", JOptionPane.ERROR_MESSAGE);								
						}
						else
						{
							this.takeShot(i,j);
					...
					{
						JOptionPane.showMessageDialog(null,"You fired on your own board!","Lost Turn",JOptionPane.WARNING_MESSAGE);
						break outer;							
					}						
				}
			}
			...
			if ((i == 10)&&(j == 10))
			{
				JOptionPane.showMessageDialog(null,"You took too long!", "Lost Turn",JOptionPane.INFORMATION_MESSAGE);
			}
			...
			{
				if (Battleship.getPlayers(Battleship.getEnemy()).getUser().equals("Computer"))//change once menu options work
				{
				...
				else
				{
					...
					this.getUser()+" won!!!","It's A Celebration!", JOptionPane.INFORMATION_MESSAGE);							
				}						
			}									
		}						
	}
	...
	/**
	 * @author 432873
	 * Listener for the buttons on the board. Only works while the game is in session
	 */
		...
		else
		{
			if (this.getUser().equals("Computer"))
			{
				...
				for (i = 0; i < 10; i++)
				{
					for (j = 0; j < 10; j++)
					{
						if ((!this.getWhatShip(i,j).equals(" ")) && (((this.getBboard(i,j).getBackground())!=Color.black) && ((this.getBboard(i,j).getBackground())!=Color.orange)))							
							{
			...
			{
				JOptionPane.showMessageDialog(null,this.getUser() + " won!!!", "It's A Celebration!", JOptionPane.INFORMATION_MESSAGE);
			}
		}					
	}	
}
